...
//Create an instance of  SellRequest model
const SellRequest = mongoose.model<SellRequestI>('SellRequest', SellRequestSchema);
//index  documents to enable searching
SellRequest.createIndexes();
...
class SellRequestController {
            ...
            (err, sellRequest) => {
                if (err) {
                    ...
                    sellRequest.save(err => {
                        ...
                        return res.status(201).send({ request: sellRequest, status: "Success", code: 201, message: 'sell request successfully created' });
    ...
    public getAll(req: Request, res: Response): void {
        ...
        try {
            ...
            SellRequest.find(query, async function (err: any, items: any) {
                // get total documents in the User collection 
                const count: number = await SellRequest.countDocuments();
                ...
                const links: Record<string, unknown> = {
                    'nextLink': `${req.protocol}://${req.get('host')}/api/requests/sell?page=${nextPage}&limit=${limit}`,
                    'prevLink': `${req.protocol}://${req.get('host')}/api/requests/sell?page=${prevPage}&limit=${limit}`
                };
    ...
    public getItemWithID(req: Request, res: Response): void {
        ...
        SellRequest.findById(req.params.itemId, function (err, item) {
            if (err) {
                code = 500;
                status = "Server error";
                message = "There was a problem with the server.";
